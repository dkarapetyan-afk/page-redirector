        ...
        function compilePass(ts, isBlock) {
            ...
            while(j < ts.length) {
                ...
                if (t.type === 'NUMBER') {
                    ...
                    let start = j;
                    while (j < ts.length) {
                        if (ts[j].type === 'OPEN_BRACKET') depth++;
                        if (ts[j].type === 'CLOSE_BRACKET') {
                            if (depth === 0) break;
                            depth--;
                        }
                        j++;
                    ...
                    let start = j;
                    while (j < ts.length) {
                        if (ts[j].type === 'OPEN_BRACE') depth++;
                        if (ts[j].type === 'CLOSE_BRACE') {
                            if (depth === 0) break;
                            depth--;
                        }
                        j++;
                    ...
                    pushedTokens++;
                } else if (t.type === 'CLOSE_BRACKET' || t.type === 'CLOSE_BRACE' || t.type === 'SEMICOLON') {
