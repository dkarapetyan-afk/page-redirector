...
| `get` | `( arr n -- elem )` | Get element at index |
...
|---|---|---|---|
| `ip` | int | `0` | `tokens.length` |
| `tokens` | array | (from lexer) | — |
| `stack` | array | `[url]` | 64 entries |
| `locals` | array | `[null × 8]` | 8 slots per frame |
| `call_stack` | array of `{ip, locals}` | `[]` | depth 16 |
| `dictionary` | map of `name → {start, end}` | (built-ins) | — |
| `ops` | int | `0` | `maxOps` |
| `maxOps` | int | `1024` | — |
...
```forth
( Remove any query param starting with "utm_" )
dup param-keys
[ "utm_" starts-with not ] filter
( stack: url clean-keys )
drop redirect
```
